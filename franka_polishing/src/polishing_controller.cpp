...
void PolishingController::update(const ros::Time& /*time*/, const ros::Duration& /*period*/) {
    ...
    Eigen::Map<Eigen::Matrix<double, 6, 1>> EE_wrench_franka(robot_state.K_F_ext_hat_K.data());  // Estimated external wrench (force, torque) acting on stiffness frame, expressed relative to the stiffness frame
    Eigen::Map<Eigen::Matrix<double, 6, 1>> O_wrench_franka(robot_state.O_F_ext_hat_K.data());  // Estimated external wrench (force, torque) acting on stiffness frame, expressed relative to the base frame
...
// ----------------------------------------------------------------------------
void PolishingController::wrenchPublisherCallback(ros::Publisher& wrench_pub, Eigen::Map<Eigen::Matrix<double, 6, 1>>& wrench) {
