#include <franka_udrilling/cartesian_impedance_controller.h>
...
void CartesianImpedanceController::starting(const ros::Time& /*time*/) {
  ...
  file_force_gains.close();

  // position proporcional force gain
  Kf << Kfx,   0,   0,
          0, Kfy,   0,
          0,   0, Kfz;
...
void CartesianImpedanceController::update(const ros::Time& /*time*/, const ros::Duration& /*period*/) {
  ...
  Eigen::Vector3d F_EE;
  F_EE << EE_force[0], EE_force[1], EE_force[2];
  ...
  // ---------------------------------------------------------------------------
  // Set the force controller gains
  // ---------------------------------------------------------------------------
  cartesian_Kf_.topLeftCorner(3, 3) << Kf;

  // ---------------------------------------------------------------------------
  // compute force error
  // ---------------------------------------------------------------------------
  force_error.head(3) << F_EE_d_ - F_EE;
  force_error.tail(3) << 0.0, 0.0, 0.0;
  ...
  tau_task << jacobian.transpose() * ( cartesian_damping_ * velocity_error + cartesian_stiffness_ * error + cartesian_integral_ * integral_error + cartesian_Kf_ * force_error );
...
PLUGINLIB_EXPORT_CLASS(franka_udrilling::CartesianImpedanceController,
